<?php
	...
	***/
	...
	/***
	 ***	@Error handling: access Code not valid
	 ***/
	add_action('um_submit_form_errors_hook__wrongaccesscode', 'um_submit_form_errors_hook__wrongaccesscode', 10);
	function um_submit_form_errors_hook__wrongaccesscode($args) {
		global $ultimatemember;
	    global $wpdb;
	    $password = $args["user_password"];
	    if (isset( $args['user_password']) && isset( $args['role'])){
	    	if ($args['role'] == 'donor'){
	    		$myrows = $wpdb->get_results( "SELECT * FROM mm_donor_image_meta WHERE code='$password'");
	    		$num_rows = count($myrows);
	    		if ($num_rows == 0) {
	    			$ultimatemember->form->add_error( "user_password",  __('The code is invalid','ultimatemember') );
	    		} else {
	    			if (($myrows[0]->wp_id) != 0){
	    				$user = get_userdata(  $myrows[0]->wp_id);
	    			    $ultimatemember->form->add_error( "user_password",  __('You are already registered, switch to the login to enter!','ultimatemember') );
	    			}
	    		}
	    	}
	    }		
	...
	***/
	...
	function um_submit_form_errors_hook( $args ){
		...
		if ( $mode == 'register' ){
			...
			do_action("um_submit_form_errors_hook__wrongaccesscode", $args);
